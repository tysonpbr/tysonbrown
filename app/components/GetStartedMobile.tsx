...
const GetStartedMobile = ({
  ...
  return (
    <div className={`${location != 'Home' ? 'opacity-0' : 'opacity-100'} delay-300 ease-in-out duration-500 lg:hidden fixed top-0 left-0 w-screen h-[100svh] text-black sepia-50 pointer-events-none`}>
          ...
          {showText &&
            paragraph.split(' ').map((word, index) => (
              <span
                ...
                style={{ animationDelay: `${index * 0.15 + 1.6}s` }}
              >
                {word + '\u00A0'}
              </span>
            ))}
